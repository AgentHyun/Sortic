...
export const addSorterAction = atom(null, async (get, set) => {
  ...
  try {
    // 정렬자 추가 요청
    const response = await authAxios.post('/sorter/add', newSorter);

    // 서버에서 최신 정렬자 목록을 가져와서 상태 업데이트
    const updatedSortersResponse = await authAxios.get(`/sorter/user/${userId}`);
...
export const deleteSorterAction = atom(null, async (get, set, sorterIdToDelete) => {
  ...
  try {
    // 정렬자 삭제
    await authAxios.post('/sorter/delete', {
    ...
    // 백엔드에 재정렬 요청
    const reordered = await authAxios.post('/sorter/reorder', renamed);
...
export const fetchSortersByUserAction = atom(null, async (get, set) => {
  ...
  try {
    const response = await authAxios.get(`/sorter/user/${userId}`);
...
export const updateSorterNameAction = atom(null, async (get, set, { oldSorterName, newName }) => {
  try {
    const response = await authAxios.put('/sorter/update', {
      oldSorterName,
      sorterName: newName,
    }, {
      headers: {
...
export const deleteMultipleSortersAction = atom(null, async (get, set, sorterIdsToDelete) => {
  ...
  try {
    await authAxios.post('/sorter/delete/multiple', sorterIdsToDelete);
    ...
    const reordered = await authAxios.post('/sorter/reorder', renamedWithUserId);
...
export const getElementNameByIdAction = atom(
  ...
  async (get, set, elementsId) => {
    try {
      const response = await authAxios.get(`/elements/${elementsId}`);
...
export const getSorterIdByNameAndElementIdAction = atom(
  ...
  async (get, set, { sorterName, elementsId }) => {
    try {
      const response = await authAxios.get('/sorter/get-sorter-id', {
        params: {
          sorterName,
          elementsId
        }
...
export const getElementsIdBySorterIdAction = atom(
  ...
  async (get, set, sorterId) => {
    try {
      // 수정된 API 경로 사용
      const response = await authAxios.get(`/sorter-element/sorter/${sorterId}`);
...
export const getSorterNameByIdAction = atom(
  ...
  async (get, set, sorterId) => {
    try {
      const response = await authAxios.get(`/sorter/name/${sorterId}`);
